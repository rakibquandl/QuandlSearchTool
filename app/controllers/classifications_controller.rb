class ClassificationsController < ApplicationController
  ...
  def destroy
    unclassified_id = Classification.find_by_value('Unclassified').id
    @classification = Classification.find(params[:id])

    SearchItem.where(classification_id: @classification.id).each do |i|
          i.classification_id = unclassified_id
          i.save
    end
